...
request.onupgradeneeded = function(event) {
  ...
  // create an object store (table) called `new_budget`, set it to auto increment the primary key
...
request.onsuccess = function(event) {
  ...
  //(from onupgradeneeded event above) or simply established 
...
// This function will be executed if we attempted to 
// submit a new budget item and there is no internet connection
function saveRecord(record) {
  // open a new transaction with the database with read and write permissions
  const transaction = db.transaction(['new_budget'], 'readwrite');

  const budgetItemObjectStore = transaction.objectStore('new_budget');
